...
class Ui_scope(object):
    ...
    def get_hor_delay(self):
        if (self.lineEdit_8.text().split(".")[0]).isdigit() or self.lineEdit_8.text().isdigit() or \
                (self.lineEdit_8.text().split('-')[-1]).split(".")[-1].isdigit():
    ...
    def hor_get_scale(self):
        if (self.lineEdit_9.text().split(".")[0]).isdigit() or self.lineEdit_9.text().isdigit() or \
                (self.lineEdit_9.text().split('-')[-1]).split(".")[-1].isdigit():
    ...
    def impandence_ch1_50(self):
        reply = QMessageBox.warning(None, '提示', '确认输入电压是小于5V，点击\"YES\"继续。',
    ...
    def get_scale(self):
        if (self.lineEdit.text().split(".")[0]).isdigit() or self.lineEdit.text().isdigit() or \
                (self.lineEdit.text().split('-')[-1]).split(".")[-1].isdigit():
        ...
        if (self.lineEdit_2.text().split(".")[0]).isdigit() or self.lineEdit_2.text().isdigit() or \
                (self.lineEdit_3.text().split('-')[-1]).split(".")[-1].isdigit():
        ...
        if (self.lineEdit_3.text().split(".")[0]).isdigit() or self.lineEdit_3.text().isdigit() or \
                (self.lineEdit_3.text().split('-')[-1]).split(".")[-1].isdigit():
        ...
        if (self.lineEdit_4.text().split(".")[0]).isdigit() or self.lineEdit_4.text().isdigit() or \
                (self.lineEdit_4.text().split('-')[-1]).split(".")[-1].isdigit():
                ...
                if type in ['RMS', 'MAX', 'MIN', 'MEAN', 'PKPK']:
                    if -1 < float(measure_data) < 1:
                        ...
                        self.textBrowser.append(
                            '通道{}\t'.format(channel) + type + '：  ' + str(round(float(measure_data), 3)) + 'mV')
                        self.textBrowser.ensureCursorVisible()
                    else:
                        self.textBrowser.append(
                            '通道{}\t'.format(channel) + type + '：  ' + str(round(float(measure_data), 3)) + 'V')
                        self.textBrowser.ensureCursorVisible()
                elif type == 'FREQ':
                    measure_data = float(measure_data)
                    self.textBrowser.append('通道{}\t'.format(channel) + type + '：  ' + str(
                        round(float(measure_data) / 1000000, 3)) + 'MHz')
                    self.textBrowser.ensureCursorVisible()
                elif type in ['PER', 'RISE10T90', 'FALL90T10']:
                    measure_data = '{:.10f}'.format(float(measure_data))
                    if float(measure_data) > 1:
                        self.textBrowser.append(
                            '通道{}\t'.format(channel) + type + '：  ' + str(round(float(measure_data), 3)) + 's')
                        self.textBrowser.ensureCursorVisible()
                    elif 0.001 < float(measure_data) < 1:
                        self.textBrowser.append('通道{}\t'.format(channel) + type + '：  ' + str(
                            round(float(measure_data) * 1000, 3)) + 'ms')
                        self.textBrowser.ensureCursorVisible()
                    elif 0.000001 < float(measure_data) < 0.001:
                        self.textBrowser.append('通道{}\t'.format(channel) + type + '：  ' + str(
                            round(float(measure_data) * 1000000, 3)) + 'us')
                        self.textBrowser.ensureCursorVisible()
                    elif 0.000000001 < float(measure_data) < 0.000001:
                        self.textBrowser.append('通道{}\t'.format(channel) + type + '：  ' + str(
                            round(float(measure_data) * 1000000000, 3)) + 'ns')
                        self.textBrowser.ensureCursorVisible()
                else:
